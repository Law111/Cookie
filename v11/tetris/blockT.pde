class blockT extends block{
  void setup(int r, int c){
    ...
    bArray[0].setup(r,c+1,colorId);
    bArray[1] = new blockPiece();
    bArray[1].setup(r,c,colorId);
   ...
   if(cases==1){ // T rotated 90 cc 2>>1
     if(bArray[0].canMove(-1+row,-1+col) && bArray[1].canMove(0+row,0+col) && bArray[2].canMove(-1+row,1+col) && bArray[3].canMove(1+row,1+col)){
     ...
     row=0;
     col=0;
   } else if(cases==2){ // T rotated 180 cc 1>>0
     if(bArray[0].canMove(1+row,-1+col) && bArray[1].canMove(0+row,0+col) && bArray[2].canMove(-1+row,-1+col) && bArray[3].canMove(-1+row,1+col)){
        bArray[0].setup(1+r0+row,-1+c0+col,colorId);
        bArray[1].setup(0+r1+row,0+c1+col,colorId);
        bArray[2].setup(-1+r2+row,-1+c2+col,colorId);
     ...
     row=0;
     col=0;
   } else if(cases==3){ // T rotated 270 cc 0>>3
     if(bArray[0].canMove(1+row,1+col) && bArray[1].canMove(0+row,0+col) && bArray[2].canMove(1+row,-1+col) && bArray[3].canMove(-1+row,-1+col)){
        bArray[0].setup(1+r0+row,1+c0+col,colorId);
        bArray[1].setup(0+r1+row,0+c1+col,colorId);
        bArray[2].setup(1+r2+row,-1+c2+col,colorId);
     ...
     row=0;
     col=0;
   } else if(cases==4){ // T rotated 360 cc 3>>2
     if(bArray[0].canMove(-1+row,1+col) && bArray[1].canMove(0+row,0+col) && bArray[2].canMove(1+row,1+col) && bArray[3].canMove(1+row,-1+col)){
        bArray[0].setup(-1+r0+row,1+c0,colorId);
        bArray[1].setup(0+r1+row,0+c1,colorId);
        bArray[2].setup(1+r2+row,1+c2,colorId);
        bArray[3].setup(1+r3+row,-1+c3,colorId);
        cases=1;
        t4=0;
      } else {
        t4++;
